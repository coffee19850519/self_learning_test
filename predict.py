...
def predict(east_detect, img_path, iteration_num, quiet=False):
    if iteration_num != 0:
        img = image.load_img(
            img_path[:-4] + '_' + str(iteration_num - 1) + '.png')
    else:
    ...
    y[:, :, :3] = sigmoid(y[:, :, :3])
    cond = np.greater_equal(y[:, :, 0], cfg.pixel_threshold - 0.1 *
    iteration_num)
    ...
    with Image.open(img_path) as im:
        ...
        im.save(img_path + '_act_'+ str(iteration_num)  +'.jpg')
        ...
        for score, geo, s in zip(quad_scores, quad_after_nms,
            ...
            if np.amin(score) > 0:
                ...
                rescaled_geo_list = np.reshape(rescaled_geo, (8,)).astype(
                    np.int32).tolist()
                txt_item = ','.join(map(str, rescaled_geo_list))
                txt_items.append(txt_item + '\n')
                #erase detextive text to background color
                '''
                quad_draw.rectangle((tuple(geo[0] - cfg.erase_offset_pixels),
                                     tuple(geo[2] + cfg.erase_offset_pixels)),
                                    fill='white',
                                    width=-1)'''
                #quad_draw.text(tuple(geo[1]+3),str(score))

            elif not quiet:
                print('quad invalid with vertex num less then 4.')
        #quad_im.save(img_path[:-4] + '_predict_'+ str(iteration_num) +'.jpg')
        if cfg.predict_write2txt and len(txt_items) > 0:
            with open(img_path[:-4] +'.txt', 'a') as f_txt:
...
if __name__ == '__main__':
    ...
    predict(east_detect, img_path, 0)
